...
//! （動画秒を持つか → 時刻 → phase 開始か → recordedAt → id — `persistence_order`）でソート済みである前提
//! （入力契約 — ADR 0001）。
...
pub use fact_validator::{
    RosterContext, has_anchor_mismatched_with_configuration, validate_control_fact,
    validate_match_fact, validate_play_fact, validate_possession_fact,
};
