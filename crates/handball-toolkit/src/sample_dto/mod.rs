...
//! 将来の JSON 検証 CLI・wasm デモの土台）。export 方向（domain → DTO + Swift JSONEncoder
//! 互換の encode）は ADR 0004 決定 2 による新規実装で、以後 SAMPLE_DTO_V2 schema の変更は
//! この 1 箇所の修正で全シェルに波及する。
//!
...
mod sample_match_encoder;
mod sample_match_exporter;

pub use sample_match_converter::{
    SampleMatchConversionResult, convert, decode_configuration, decode_fact, required_id_count,
};
