...
use crate::persistence_order::sort_by_persistence_order;
...
/// facts のみ **永続化順へ整列する**（規約は `persistence_order` モジュール。整列する理由は
/// `import_commit_plan` の該当箇所 — 移植元から意図的に乖離。handball-project#72）。
#[derive(Debug, Clone, PartialEq)]
...
pub fn import_commit_plan(
    ...
    // `.video` サンプルは phase 開始（videoClock 1086s）が配列の 38 番目にあり、それより後ろの
    // 時刻（1130s〜）の play が先に並んでいる。
    //
    // 一方 `commit_sample_match_import` は facts を 1 件ずつ `record_append_fact` で発火し、
    // その都度 **whole-log 検証**（`validate_fact_log`）を通す。この検証は「facts は永続化順で
    // 並んでいる前提」で書かれており、かつ R3 / R5 は「fact が 1 件以上あって PhaseStart が
    // 無い」だけで落ちる。そのため DTO 順のまま append すると、`.video` は最初の play を積んだ
    // 時点で `VideoWithFactsMissingPhaseStart` により必ず失敗していた（handball-project#72）。
    //
