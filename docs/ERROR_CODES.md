...
> **Never show the `detail` fields to users.** They are diagnostic strings for
> developers, as is the `Display` implementation of these types. Branch on the error
> case and supply your own wording (ADR 0002).

> The diagnostic field is called `detail`, not `message`, on purpose. In Kotlin these
> errors are generated as `sealed class … : kotlin.Exception()` subclasses, and a field
> named `message` collides with `Throwable.message` badly enough that the generated
> bindings do not compile. Keep that in mind if you add a case here.
...
| `Repository` | `detail: string` | The repository you injected failed. Also carries any exception your implementation threw. |
...
| `MigrationPlanInfeasible` | `detail: string` | A video migration could not be planned. A safety net; the wizard's own validation should prevent it. |
| `ImportDecodeFailed` | `detail: string` | An import payload could not be decoded into domain types. |
| `VideoSourceNotReplaceable` | `configuration: MatchConfigurationKind` | The match has no video source to replace (it is `timer`). Switching a timer match to video needs sync points — use the migration entry point instead. |
| `PhaseDurationChangeInfeasible` | `detail: string` | A phase-duration change could not be planned (not a timer match, the fact is missing or is not a phase start, or the duration is not positive). A safety net; picking a phase fact in the UI should prevent it. |
| `PhaseDurationClampNotPermitted` | `clampedFactCount: u32` | Shortening the phase would push this many records past its new end, and you did not permit moving them. Show the count, get the user's approval, then call again with `allowClamp` set. Nothing was persisted. |
...
| case | fields | meaning |
|---|---|---|
| `InvalidJson` | `detail: string` | The JSON does not parse as the expected schema. |
