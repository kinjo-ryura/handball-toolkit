...
```sh
mkdir -p app/libs
gh release download v0.10.0 --pattern '*.aar' --dir app/libs
```
...
cp target/aar/handball-toolkit-0.10.0.aar examples/android/app/libs/
```
...
| Room | 2.7.2 | |
| handball-toolkit | 0.10.0 | `app/libs/handball-toolkit-0.10.0.aar`。コア crate の version に従う |
