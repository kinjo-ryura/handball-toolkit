...
**fact 列は永続化順（動画秒を持つ fact が先 → 動画秒、無ければ累積秒 → phase 開始か → recordedAt → id）にソートしてから**渡すこと。未ソートのまま渡してもエラーにはならず、**黙って誤った結果を返す**。
...
```bash
./scripts/build_aar.sh                     # → target/aar/handball-toolkit-<version>.aar
gh release create v0.11.0 target/aar/handball-toolkit-0.11.0.aar \
  --title "v0.11.0" --notes "..."
```
