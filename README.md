...
```kotlin
dependencies {
    implementation(files("libs/handball-toolkit-0.10.0.aar"))
...
|---|---|
| コア crate | `0.10.0` |
| `.aar` ファイル名 | `handball-toolkit-0.10.0.aar` |
| git タグ / Release | `v0.10.0` |
...
```bash
./scripts/build_aar.sh                     # → target/aar/handball-toolkit-<version>.aar
gh release create v0.10.0 target/aar/handball-toolkit-0.10.0.aar \
  --title "v0.10.0" --notes "..."
```
...
配布バイナリ（iOS の staticlib / Android の `.so` / Web の `.wasm`）には MIT / MPL-2.0 / Unicode-3.0 の OSS がリンクされる。**Executable Form で配る側にはライセンス本文と著作権表示を受領者へ届ける義務がある**（MPL-2.0 は加えて §3.2 の「ソース入手方法の告知」）。このリポはその一覧を [`THIRD_PARTY_LICENSES.json`](THIRD_PARTY_LICENSES.json) として持ち、**シェル側はこれを同梱して画面に出す**。同じ内容を人が読める形にした [`THIRD_PARTY_LICENSES.md`](THIRD_PARTY_LICENSES.md) も置いており、画面に一覧を持たない配布経路はこれへリンクして表示を届けられる。

```bash
./scripts/generate_licenses.sh           # JSON と Markdown を生成して書き出す
./scripts/generate_licenses.sh --check   # 両方が依存の現況と一致するか検査（CI が実行）
```

- 一覧は `cargo-about` が Cargo.lock から起こす。**手で書かない・手で直さない**。依存を足したら再生成してコミットする（忘れても CI の `--check` が落ちる）
- **Markdown は JSON だけから機械的に作る**（`render_markdown`。cargo を呼ばない）。JSON の写しなので食い違う余地が無く、版を上げて JSON が変われば Markdown も同じ生成で変わる。**リンクする側は写しを持たずに済む** — Web デモ（handball-apps-site）は以前、親リポでこの JSON から一覧ページを生成していたが、リリースで版表記が変わるたびに古くなった（handball-project#384）
...
iOS は `HandballRecorder` の `Packages/HandballToolkit/bootstrap.sh` がこの JSON をパッケージリソースへ取り込み、生成 Swift と同じく一致検証する。Web デモ（handball-apps-site）は `handball-recorder/licenses/` から main の `THIRD_PARTY_LICENSES.md` へリンクする（handball-project#301 / #384）。Android は `scripts/build_aar.sh` が `.aar` の `assets/handball_toolkit/third-party-licenses.json` へ同梱する（handball-project#142）。**`.aar` を受け取った側に何の義務が移るか**は「Android」節の「同梱される OSS ライセンス一覧」を参照。
